...
<div class="dealFrame <?php print ($node->deal_status == 'active' ? 'onSale' : '');?>">
  <div class="dealHeader <?php print ($node->deal_status == 'ended' ? 'ended' : '');?>"><?php ($node->deal_status == 'upcoming' ? print t('Deal Starts on ') . date("ga T", strtotime($node->field_start_time['und'][0]['value'])) : ($node->deal_status == 'active' ? print t('On Sale Now') : print t('This sale has ended'))) ?></div>
    <div class="pricing">
